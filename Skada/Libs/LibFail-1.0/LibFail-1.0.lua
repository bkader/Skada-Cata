local MAJOR, MINOR = "LibFail-1.0", tonumber("259") or 999
...
local snare_effects = {
	[GetSpellInfo(65857)] = true, -- Entangling Roots
	[GetSpellInfo(66071)] = true, -- Natures Grasp
	[GetSpellInfo(65545)] = true, -- Psychic Horror
	[GetSpellInfo(65543)] = true, -- Psychic Scream
	[GetSpellInfo(65792)] = true, -- Frost Nova
	[GetSpellInfo(65809)] = true, -- Fear
	[GetSpellInfo(66613)] = true, -- Hammer of Justice
	[GetSpellInfo(65930)] = true, -- Intimidating Shout
	[GetSpellInfo(65880)] = true, -- Frost Trap
	[GetSpellInfo(66207)] = true, -- Wing Clip
	[GetSpellInfo(66020)] = true, -- Chains of Ice
--- ICC
	[GetSpellInfo(71615)] = true, -- Tear Gas -- Professor Putricide
	[GetSpellInfo(70447)] = true, -- Volatile Ooze Adhesive -- Professor Putricide
}

function lib:IsSnared(target)
	for debuff in next, snare_effects do
		if UnitDebuff(target, debuff) == debuff then
			return true
		end
	end
	return false
...
function lib:InitVariables()
	...
	self.ChargeCounter = wipe(self.ChargeCounter or {})
	...
	self.DeathTime = 0
	self.RaidTable = wipe(self.RaidTable or {}) -- Mostly for the Auriaya fail, but could be usefull
	self.SindragosaSingleBeacon = 0
	self.SindragosaBeaconTarget = nil
	self.TheLichKingNecroticPlagueTarget = wipe(self.TheLichKingNecroticPlagueTarget or {})
	...
	self.ValithriaEmeraldVigor = wipe(self.ValithriaEmeraldVigor or {})

	self.LastEvent = wipe(self.LastEvent or {})

	if not self.targetlist then
		self.targetlist = {"target", "targettarget", "focus", "focustarget", "mouseover", "mouseovertarget"}
		for i = 1, 4 do self.targetlist[#self.targetlist+1] = ("boss%d"):format(i) end
		for i = 1, 4 do self.targetlist[#self.targetlist+1] = ("party%dtarget"):format(i) end
		for i = 1, 40 do self.targetlist[#self.targetlist+1] = ("raid%dtarget"):format(i) end
	end

	-- Last whatever
	for i=1, #fail_events do
		self.LastEvent[fail_events[i]] = wipe(self.LastEvent[fail_events[i]] or {})
	end

end
lib.PLAYER_REGEN_ENABLED = lib.InitVariables
...
function lib:CancelTimer(name)
	if not name then
		self.timers = wipe(self.timers or {})
		return frame:Hide()
	end

	self.timers[name] = del(self.timers[name], true)
	...
	frame:SetScript("OnEvent", function (self, event, ...)
		if event == "COMBAT_LOG_EVENT_UNFILTERED" then
			...
			if etype == "SPELL_MISSED" then  -- lets hack the misses onto the damage event
				local timestamp, _, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, missType,  amountMissed = ...
				local damage, overkill = 0, 0

				lib.SPELL_DAMAGE(lib, timestamp, etype, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, damage, overkill, missType, amountMissed)
...
lib.RAID_ROSTER_UPDATE = lib.PLAYER_ENTERING_WORLD
...
local function EmalonChainLightning()
	lib.ThreePeopleHugging = false
end
local ThorimChainLightning = EmalonChainLightning

function lib:SPELL_DAMAGE(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, damage, overkill)
   ...
   -- Gunship -- Explosion (it's still avoidable so optional fail for normal mode)
	...
	if spellId == 19983 and is_playerevent and overkill > 0 then
		if GetMobId(sourceGUID) ~= 36853 then return end
	...
	if (spellId == 64213 or spellId == 64215) and is_playerevent then
		if damage >= 10000 then -- at least 3 ppl hugging, and I think highest damage appears in combat log first always (Maat @Ensidiafails)
			...
			self:ScheduleTimer("EmalonChainLightning", EmalonChainLightning, 3)
		end
	...
	if spellId == 15284 and overkill > 0 and is_playerevent then
		if GetMobId(sourceGUID) ~= 12129 then return end
	...
	if (spellId == 61878 or spellId == 63480) and is_playerevent and damage >= self.COUNCIL_OVERLOAD_THRESHOLD then -- DKs with AMS shouln't fail
		if GetMobId(sourceGUID) ~= 32857 then return end
   ...
   -- Thorim Lightning Charge
	if spellId == 62466 and is_playerevent then
		if GetMobId(sourceGUID) ~= 32865 then return end -- it's not from Thorim
	...
	if (spellId == 64390 or spellId == 62131) and is_playerevent then
		if damage > 10000 then
			self.ThreePeopleHugging = true
			self:ScheduleTimer("ThorimChainLightning", ThorimChainLightning, 3)
		end
	...
	if (spellId == 63009 or spellId == 66351) and is_playerevent then
		if GetMobId(sourceGUID) ~= 34362 then return end -- Explosion not from mines
	...
	if (spellId == 65333 or spellId == 64626) and is_playerevent then
		if GetMobId(sourceGUID) ~= 34149 then return end --Explosion not from Frost Bomb
   ...
   if spellId == 63801 and is_playerevent and damage > 0 then
	...
	if spellId == 19983 and overkill > 0 and is_playerevent then
		if GetMobId(sourceGUID) ~= 15989 then return end
...
function lib:SWING_DAMAGE(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, damage, overkill)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
	damage = damage ~= "ABSORB" and damage or 0
	overkill = overkill or 0
...
--12/26 20:27:36.560  SWING_DAMAGE,0xF13000966C005B92,"Blood Beast",0x10a48,0x040000000396C708,"Outofcontrol",0x2000514,5033,0,1,0,0,1258,nil,nil,nil
	...
	if GetMobId(sourceGUID) == 38508 and is_playerevent and damage > 0 then
	...
	if GetMobId(sourceGUID) == 38222 and is_playerevent then
...
function lib:SWING_MISSED(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
...
function lib:ENVIRONMENTAL_DAMAGE(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, dmgType)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
...
local function AuriayaScreech()
	for name, failed in pairs(lib.RaidTable) do
		if failed then
			lib:FailEvent("Fail_Auriaya_SonicScreech", name, lib.FAIL_TYPE_MOVING)
		else
			lib.RaidTable[name] = true
		end
	end
end
local function Algalon_BigBang()
	lib.BigbangCasting = false
end

function lib:SPELL_CAST_START(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool)
	-- local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
	...
	if spellId == 64688 or spellId == 64422 then
		self:InitRaidTable()
		self:ScheduleTimer("AuriayaScreech", AuriayaScreech, 4)
		return
	end
	...
	if spellId == 64584 or spellId == 64443 then
		...
		self:ScheduleTimer("Algalon_BigBang", Algalon_BigBang, 8)
...
function lib:UNIT_DIED(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
	local mobid = GetMobId(destGUID)
...
function lib:SPELL_PERIODIC_DAMAGE(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, damage, overkill)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
	damage = damage ~= "ABSORB" and damage or 0
	overkill = overkill or 0
...
function lib:SPELL_HEAL(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, damage)
	-- local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
...
local function Sindragosa_SingleBeacon()
	lib.SindragosaSingleBeacon = 0
end
local function Sindragosa_TombFail()
	lib:FailEvent("Fail_Sindragosa_IceTomb", lib.SindragosaBeaconTarget, lib.FAIL_TYPE_NOTMOVING)
end

function lib:SPELL_AURA_APPLIED(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, auraType)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
	...
	if spellId == 70126 and is_playerevent then
		...
		if self.SindragosaSingleBeacon == 0 then
			...
			self:ScheduleTimer("Sindragosa_SingleBeacon", Sindragosa_SingleBeacon, 10)
		end
	...
	elseif spellId == 70157 and is_playerevent then
		...
		self:ScheduleTimer("Sindragosa_TombFail", Sindragosa_TombFail, 0.2)
...
function lib:SPELL_AURA_APPLIED_DOSE(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, auraType, amount)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
...
function lib:SPELL_INTERRUPT(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, extraSpellId)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
...
function lib:SPELL_SUMMON(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName)
	-- local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
...
function lib:SPELL_CAST_SUCCESS(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool)
...
function lib:SPELL_DISPEL(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, extraSpellId, extraSpellName)
...
function lib:SPELL_AURA_REMOVED(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
...
function lib:SPELL_PERIODIC_ENERGIZE(timestamp, type, _, sourceGUID, sourceName, sourceFlags, _, destGUID, destName, destFlags, _, spellId, spellName, spellSchool, amount, powerType)
	local is_playerevent = bit.band(destFlags or 0, COMBATLOG_OBJECT_TYPE_PLAYER) > 0
