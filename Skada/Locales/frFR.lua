--[[ Translator: Kader B ]] --
local L = LibStub("AceLocale-3.0"):NewLocale("Skada", "frFR")
if not L then return end

L["A damage meter."] = "Un \"damage meter\"."
L["Memory usage is high. You may want to reset Skada, and enable one of the automatic reset options."] = "L'utilisation de la mémoire est élevée. Vous devriez réinitialiser Skada et activer une des options de réinitialisation automatique."
L["Skada is out of date. You can download the newest version from |cffffbb00%s|r"] = "Skada n'est pas à jour. Vous pouvez télécharger la nouvelle version sur |cffffbb00%s|r"
L["Skada: Modes"] = "Skada: Modes"
L["Skada: Fights"] = "Skada : Combats"
L["Data Collection"] = "Collecte de données"
L["ENABLED"] = "ACTIVÉ"
L["DISABLED"] = "DÉSACTIVÉ"
L["Stopping for wipe."] = "Arrêt pour cause de wipe."
L["Usage:"] = "Usage:"
-- profiles
L["Profiles"] = "Profils"
L["Profile Import/Export"] = "Importation/Exportation de profils"
L["Import Profile"] = "Importer un profil"
L["Export Profile"] = "Exporter le profil"
L["Paste here a profile in text format."] = "Collez ici un profil au format texte."
L["Press CTRL-V to paste a Skada configuration text."] = "Ctrl-V pour coller un texte de config. Skada."
L["This is your current profile in text format."] = "Voici votre profil actuel au format texte."
L["Press CTRL-C to copy the configuration to your clipboard."] = "Ctrl-C pour copier la config. dans votre presse-papier."
-- common lines
L["Options"] = "Paramètres"
L["Options for %s."] = "Paramètres \1242 %s."
L["General"] = "Général"
L["General options for %s."] = "Paramètres généraux \1242 %s."
L["Text"] = "Texte"
L["Text options for %s."] = "Paramètres de texte \1242 %s."
L["Format"] = "Format"
L["Format options for %s."] = "Options de format \1242 %s."
L["Appearance"] = "Apparence"
L["Appearance options for %s."] = "Options d'apparence \1242 %s."
L["Advanced"] = "Avancé"
L["Advanced options for %s."] = "Paramètres avancés \1242 %s."
L["Position"] = "Position"
L["Position settings for %s."] = "Paramètres de position \1242 %s."
L["Width"] = "Largeur"
L["The width of %s."] = "La largeur \1242 %s."
L["Height"] = "Hauteur"
L["The height of %s."] = "La hauteur \1242 %s."
L["Verbose Mode"] = "Mode verbeux"
L["Enable verbose mode for %s."] = "Activer le mode verbeux pour %s."
L["More Details"] = "Plus de détails"
L["Active Time"] = "Temps d'activité"
L["Segment Time"] = "Temps du segment"
L["Click for |cff00ff00%s|r"] = "Cliquer pour |cff00ff00%s|r"
L["Shift-Click for |cff00ff00%s|r"] = "Shift-clic pour |cff00ff00%s|r"
L["Control-Click for |cff00ff00%s|r"] = "Ctrl-clic pour |cff00ff00%s|r"
L["Alt-Click for |cff00ff00%s|r"] = "Alt-clic pour |cff00ff00%s|r"
L["Toggle Class Filter"] = "Filtrer par classe"
L["Average"] = "Moyenne"
L["Minimum Hit"] = "Coup minimum"
L["Maximum Hit"] = "Coup maximum"
L["Average Hit"] = "Coup moyen"
L["Casts"] = "Incantations"
L["Count"] = "Nombre"
L["Refresh"] = "Réactualisation"
L["Percent"] = "Pourcentage"
L["sPercent"] = "Pourcentage (sous-vues)"
L["General Options"] = "Paramètres généraux"
L["HoT"] = " (HoT)"
L["DoT"] = " (DoT)"
L["Hits"] = "Coups"
L["Normal Hits"] = "Coups normaux"
L["Critical"] = "Critique"
L["Critical Hits"] = "Coups critiques"
L["Crushing"] = "Écrasement"
L["Glancing"] = "Érafle"
L["ABSORB"] = "Absorbé"
L["BLOCK"] = "Bloqué"
L["DEFLECT"] = "Dévie"
L["DODGE"] = "Esquive"
L["EVADE"] = "Évite"
L["IMMUNE"] = "Insensible"
L["MISS"] = "Raté"
L["PARRY"] = "Pare"
L["REFLECT"] = "Renvoyé"
L["RESIST"] = "Résisté"
L["Only for bosses."] = "Uniquement contre les boss"
L["Enable this only against bosses."] = "Activer uniquement contre les boss."
-- windows section:
L["Window"] = "Fenêtre"
L["Windows"] = "Fenêtres"
L["Create Window"] = "Créer une fenêtre"
L["Window Name"] = "Nom de la fenêtre"
L["Enter the name for the new window."] = "Entrez le nom de la nouvelle fenêtre"
L["Delete Window"] = "Supprimer la fenêtre"
L["Choose the window to be deleted."] = "Choisissez la fenêtre à supprimer"
L["Are you sure you want to delete this window?"] = "Êtes-vous sûr de vouloir supprimer cette fenêtre?"
L["Rename Window"] = "Renommer la fenêtre"
L["Enter the name for the window."] = "Entrez le nom de la fenêtre"
L["Test Mode"] = "Mode de test"
L["Creates fake data to help you configure your windows."] = "Crée de fausses données pour vous aider à configurer vos fenêtres."
L["Child Window"] = "Fenêtre enfant"
L["A child window will replicate the parent window actions."] = "Une fenêtre enfant copiera les actions de la fenêtre parente."
L["Child Window Mode"] = "Mode Parent-Enfant"
L["Lock Window"] = "Verrouiller la fenêtre"
L["Locks the bar window in place."] = "Verrouille la fenêtre des barres à sa position actuelle."
L["Hide Window"] = "Cacher la fenêtre"
L["Hides the window."] = "Cache la fenêtre"
L["Sticky Window"] = "Fenêtre collante"
L["Allows the window to stick to other Skada windows."] = "Permet à la fenêtre de s'aligner sur d'autres fenêtres Skada."
L["Snap to best fit"] = "Corriger pour adapter"
L["Snaps the window size to best fit when resizing."] = "Corrige la taille de la fenêtre afin qu'elle s'adapte mieux à son contenu lors des redimensionnements."
L["Disable Resize Buttons"] = "Désactiver les boutons de redimensionnements"
L["Resize and lock/unlock buttons won't show up when you hover over the window."] = "Les boutons de redimensionnement et de verrouillage/déverrouillage ne s'afficheront pas lorsque vous survolez la fenêtre."
L["Display System"] = "Système d'affichage"
L["Choose the system to be used for displaying data in this window."] = "Choisissez le système à utiliser pour l'affichage des données dans cette fenêtre."
L["Copy Settings"] = "Copier les paramètres"
L["Choose the window from which you want to copy the settings."] = "Choisissez la fenêtre à partir de laquelle vous souhaitez copier les paramètres."
-- bars
L["Bars"] = "Barres"
L["Left Text"] = "Texte de gauche"
L["Right Text"] = "Texte de droite"
L["Font"] = "Police"
L["The font used by %s."] = "La police utilisée par %s."
L["Font Size"] = "Taille de police"
L["The font size of %s."] = "La taille de police \1242 %s."
L["Font Outline"] = "Contour de la police"
L["Sets the font outline."] = "Définit le type de contour de la police."
L["Outline"] = "Contour"
L["Thick outline"] = "Contour épais"
-- L["Monochrome"] = ""
L["Outlined monochrome"] = "Contour monochrome"
L["Bar Texture"] = "Texture des barres"
L["The texture used by all bars."] = "La texture utilisée par toutes les barres."
L["Spacing"] = "Espacement"
L["Distance between %s."] = "La distance entre %s."
L["Max Bars"] = "Nbre max. de barres"
L["The maximum number of bars shown."] = "Le nombre maximal de barres à afficher."
L["Bar Orientation"] = "Orientation des barres"
L["The direction the bars are drawn in."] = "La direction vers laquelle les barres sont dessinées"
L["Left to right"] = "Gauche vers la droite"
L["Right to left"] = "Droite vers la gauche"
L["Reverse bar growth"] = "Croissance inversée des barres"
L["Bars will grow up instead of down."] = "Les barres s'ajouteront vers le haut au lieu de vers le bas."
L["Disable bar highlight"] = "Désactiver la mise en évidence de la barre"
L["Hovering a bar won't make it brighter."] = "Le survol d'une barre ne la rendra pas plus lumineuse."
L["Bar Color"] = "Couleur des barres"
L["Choose the default color of the bars."] = "Choisissez la couleur par défaut des barres."
L["Background Color"] = "Couleur du fond"
L["Choose the background color of the bars."] = "Choisissez la couleur de fond des barres."
L["Spell school colors"] = "Couleurs de sorts"
L["Use spell school colors where applicable."] = "Utiliser les couleurs des écoles de magie quand cela est possible."
L["When possible, bars will be colored according to player class."] = "Quand cela est possible les barres seront coloriées selon la classe des joueurs représentés."
L["When possible, bar text will be colored according to player class."] = "Quand cela est possible, le texte des barres sera colorié selon la classe du joueur représenté."
L["Class Icons"] = "Icônes de classe"
L["Use class icons where applicable."] = "Utiliser les icônes de classe quand cela est possible."
L["Spec Icons"] = "Icônes de spécialisation"
L["Use specialization icons where applicable."] = "Utiliser les icônes de spécialisation quand cela est possible."
L["Role Icons"] = "Icônes de rôle"
L["Use role icons where applicable."] = "Utiliser les icônes de rôle quand cela est possible."
L["Show Spark Effect"] = "Effet de lueur"
L["Click Through"] = "Clic à travers"
L["Disables mouse clicks on bars."] = "Désactiver les clics de souris sur les barres."
L["Smooth Bars"] = "Barres fluides"
L["Animate bar changes smoothly rather than immediately."] = "Animation fluide des barres."
-- title bar
L["Title Bar"] = "Barre de titre"
L["Enables the title bar."] = "Active la barre-titre."
L["Include set"] = "Inclure l'ensemble"
L["Include set name in title bar"] = "Inclure le nom d'ensemble dans la barre de titre"
L["Encounter Timer"] = "Temps de combat"
L["When enabled, a stopwatch is shown on the left side of the text."] = "Si activé, un chronomètre sera affiché à gauche du texte du titre."
L["Mode Icon"] = "Icône de module"
L["Shows mode's icon in the title bar."] = "Afficher l'icône du mode dans la barre de titre."
L["The texture used as the background of the title."] = "La texture utilisée comme fond du titre."
L["The background color of the title."] = "La couleur du fond du titre."
L["Border texture"] = "Texture de la bordure"
L["The texture used for the borders."] = "La texture utilisée pour les bordures."
L["The texture used for the border of the title."] = "La texture utilisée pour la bordure du titre."
L["Border Color"] = "Couleur de la bordure"
L["The color used for the border."] = "La couleur à utiliser pour la bordure."
L["Buttons"] = "Boutons"
L["Auto Hide Buttons"] = "Afficher au survol de la souris"
L["Show window buttons only if the cursor is over the title bar."] = "Afficher les boutons de la fenêtre uniquement si le curseur se trouve au-dessus de la barre de titre."
L["Buttons Style"] = "Style des boutons"
L["Are you sure you want to reinstall Skada?"] = "Êtes-vous sûr de vouloir réinstaller Skada?"
-- general window
L["Background Texture"] = "Texture du fond"
L["The texture used as the background."] = "La texture utilisée comme fond."
L["Tile"] = "Tuile"
L["Tile the background texture."] = "Tuile la texture du fond"
L["Tile Size"] = "Taille des tuiles"
L["The size of the texture pattern."] = "La taille du motif de la texture."
L["Background Color"] = "Couleur de fond"
L["The color of the background."] = "La couleur du fond."
L["Border"] = "Bordure"
L["Border Thickness"] = "Épaisseur de la bordure"
L["The thickness of the borders."] = "L'épaisseur des bordures."
L["Scale"] = "Échelle"
L["Sets the scale of the window."] = "Définit l'échelle de la fenêtre."
L["Strata"] = "Couche"
L["This determines what other frames will be in front of the frame."] = "Ceci détermine quels autres cadres sont placés devant le cadre."
L["Clamped To Screen"] = "Restreindre à l'écran"
L["Toggle whether to permit movement out of screen."] = "Permet ou non le déplacement de la fenêtre hors de l'écran."
L["X Offset"] = "Décalage X"
L["Y Offset"] = "Décalage Y"
-- switching
L["Mode Switching"] = "Changement de vue"
L["Combat Mode"] = "Mode en combat"
L.opt_combatmode_desc = "Passe automatiquement au segment |cffffbb00Actuel|r et au mode choisi ci-dessous quand vous entrez en combat."
L["Wipe Mode"] = "Mode wipe"
L.opt_wipemode_desc = "Passe automatiquement au segment |cffffbb00Actuel|r et à ce mode après un wipe."
L["Return after combat"] = "Retour après combat"
L["Return to the previous set and mode after combat ends."] = "Retourne au mode et à la vue précédente une fois le combat terminé."
L["Auto switch to current"] = "Retour au seg. actuel"
L.opt_autocurrent_desc = "Une fois en combat, cette fenêtre retourne automatiquement au segment |cffffbb00Actuel|r."
L["Auto Hide"] = "Masquage automatique"
L["While in combat"] = "En combat"
L["While out of combat"] = "Hors combat"
L["While not in a group"] = "Pas en groupe"
L["While inside an instance"] = "Dans une instance"
L["While not inside an instance"] = "Pas dans une instance"
L["In Battlegrounds"] = "Dans les champs de battaile"
L["Inline Bar Display"] = "Barres en ligne"
L["Inline display is a horizontal window style."] = "L'affichage en ligne est un style de fenêtre horizontale."
L["Font Color"] = "Couleur de police"
L["Font Color.\nClick \"Class Colors\" to begin."] = "Couleur de la police.\nCochez \"Couleurs de classes\" pour commencer."
L.opt_barwidth_desc = "Largeur des barres. Ceci n'est applicable que si \"Largeur de barre fixe\" est cochée."
L["Fixed bar width"] = "Largeur de barre fixe"
L.opt_fixedbarwidth_desc = "Si coché, la largeur de barre sera fixe. Autrement, la largeur de barre dépend de la largeur du texte."
L["Class Colors"] = "Couleurs de classes"
L["Use class colors for %s."] = "Utiliser les couleurs des classes pour %s."
L.opt_isusingclasscolors_desc = "Couleurs de classes:\n|cFFF58CBAKader|r - 5.71M (21.7K)\nSans:\nKader - 5.71M (21.7K)"
L["Put values on new line."] = "Valeurs sur une nouvelle ligne."
L.opt_isonnewline_desc = "Nouvelle ligne:\nKader\n5.71M (21.7K)\n\nSéparateur:\nKader - 5.71M (21.7K)"
L["Use ElvUI skin if avaliable."] = "Utiliser le skin ElvUI si disponible."
L.opt_isusingelvuiskin_desc = "Cochez pour utiliser un skin ElvUI.\nPar défaut: cochée"
L["Use solid background."] = "Utiliser un fond opaque."
L["Un-check this for an opaque background."] = "Décochez pour un fond opaque."
L["Data Text"] = "Texte de données"
L.mod_broker_desc = "Les textes de données agissent comme une source LDB. Ils peuvent être intégrés dans n'importe quel affichage LDB comme Titan Panel ou ChocolateBar. Ils ont également un fenêtre interne optionnelle."
L["Use frame"] = "Utiliser la fenêtre"
L.opt_useframe_desc = "Affiche une fenêtre autonome. Pas nécessaire si vous utilisez un affichage LDB tel que Titan Panel ou ChocolateBar."
L["Text Color"] = "Couleur du texte"
L["The text color of %s."] = "La couleur du texte \1242 %s."
L["Choose the default color."] = "Choisissez la couleur par défaut."
L["Hint: Left-Click to set active mode."] = "Astuce : clic-gauche pour activer."
L["Right-Click to set active set."] = "Clic-droit pour définir l'ensemble actif."
L["Shift+Left-Click to open menu."] = "Shift+clic-gauche pour ouvrir le menu."
-- data resets
L["Data Resets"] = "Réinitialisation"
L["Reset on entering instance"] = "En entrant en instance"
L["Controls if data is reset when you enter an instance."] = "Détermine si les données doivent être réinitialisées quand vous entrez dans une instance."
L["Reset on joining a group"] = "En rejoignant un groupe"
L["Controls if data is reset when you join a group."] = "Détermine si les données doivent être réinitialisées quand vous rejoignez un groupe."
L["Reset on leaving a group"] = "En quittant un groupe"
L["Controls if data is reset when you leave a group."] = "Détermine si les données doivent être réinitialisées quand vous quittez un groupe."
L["Ask"] = "Demander"
L["Do you want to reset Skada?\nHold SHIFT to reset all data."] = "Voulez-vous réinitialiser Skada?\nMaintenez SHIFT pour tout réinitialiser."
L["All data has been reset."] = "Toutes les données ont été réinitialisées."
L["There is no data to reset."] = "Pas de données à réinitialiser."
L["Skip reset dialog"] = "Réinitialiser sans confirmation"
L.opt_skippopup_desc = "Si activé, Skada sera réinitialisé sans afficher la boîte de confirmation."
-- general options
L["Show minimap button"] = "Bouton de la minicarte"
L["Toggles showing the minimap button."] = "Affiche ou non l'icône de la minicarte."
L["Shorten menus"] = "Menus raccourcis"
L.opt_shortmenu_desc = "Retire les menus de modes et de segments du menu Skada pour reduire son hauteur. Les menus sont toujours accessibles via les boutons de la fenêtre."
L["Transliterate"] = "Translitérer"
L["Converts Cyrillic letters into Latin letters."] = "Convertit les lettres cyrilliques en lettres latines."
L["Merge pets"] = "Fusionner familiers"
L["Merges pets with their owners. Changing this only affects new data."] = "Fusionne les familiers avec leurs maîtres respectifs. Cela n'affectera que les nouvelles données."
L["Show totals"] = "Afficher les totaux"
L["Shows a extra row with a summary in certain modes."] = "Affiche une rangée supplémentaire dans certain modes contenant un résumé."
L["Only keep boss fighs"] = "Garder uniquement les boss"
L["Boss fights will be kept with this on, and non-boss fights are discarded."] = "Les combats contre les boss seront conservés avec ceci activé, le reste sera ignoré."
L["Always keep boss fights"] = "Toujours garder les boss"
L["Boss fights will be kept with this on and will not be affected by Skada reset."] = "Les combats contre les boss sont gardés et ne seront pas affectés par une réinitialisation de Skada."
L["Always keep boss fighs"] = "Toujours garder les boss"
L["Boss fights will be kept with this on and are not affected by Skada reset."] = "Les combats contre les bosses sont conservés et ne seront pas affectés par la réinitialisation."
L["Hide when solo"] = "Cacher quand seul"
L["Hides Skada's window when not in a party or raid."] = "Cache la fenêtre de Skada quand vous n'êtes pas dans un groupe ou un raid."
L["Hide in PvP"] = "Cacher en JcJ"
L["Hides Skada's window when in Battlegrounds/Arenas."] = "Cache la fenêtre de Skada quand vous êtes dans un champ de bataille ou une arène."
L["Hide in combat"] = "Cacher en combat"
L["Hides Skada's window when in combat."] = "Cache la fenêtre de Skada quand vous êtes en combat."
L["Show in combat"] = "Afficher en combat"
L["Shows Skada's window when in combat."] = "Affiche la fenêtre de Skada quand vous êtes en combat."
L["Disable while hidden"] = "Désactiver quand caché"
L["Skada will not collect any data when automatically hidden."] = "Skada ne récoltera aucune donnée quand il est automatiquement caché."
L["Sort modes by usage"] = "Modes triés par utilisation"
L["The mode list will be sorted to reflect usage instead of alphabetically."] = "La liste des modes sera triée selon votre utilisation au lieu de l'être alphabétiquement."
L["Show rank numbers"] = "Afficher n° des rangs"
L["Shows numbers for relative ranks for modes where it is applicable."] = "Affiche les numéros des rangs relatifs pour les modes où cela est applicable."
L["Aggressive combat detection"] = "Détection agressive des combats"
L.opt_tentativecombatstart_desc = [[Skada utilise un mode conservateur (simple) de détection de combat qui fonctionne mieux en raids.
Avec cette option, Skada, tente de simuler les autres "damage meters".
Pratique en solo ou bien en donjons, inutile contre les bosses.]]
L["Autostop"] = "Arrêt automatique"
L.opt_autostop_desc = "Arrête automatiquement le segment après que la moitié du groupe soient morts."
L["Always show self"] = "Toujours afficher ma barre"
L.opt_showself_desc = "Garde le joueur affiché en dernier même s'il n'y a pas assez de place."
L["Number format"] = "Format des nombres"
L["Controls the way large numbers are displayed."] = "Détermine la façon dont les nombres sont affichés."
L["Condensed"] = "Condensé"
L["Detailed"] = "Détaillé"
L["Combined"] = "Combiné"
L["Comma"] = "Virgule"
L["Numeral system"] = "Système numérique"
L["Select which numeral system to use."] = "Sélectionnez le système numérique à utiliser."
L["Auto"] = "Auto"
L["Western"] = "Occidental"
L["East Asia"] = "Asie de l'Est"
L["Brackets"] = "Parenthèses"
L["Choose which type of brackets to use."] = "Choisissez le type de parenthèse à utiliser."
L["Separator"] = "Séparateur"
L["Choose which character is used to separator values between brackets."] = "Choisissez le caractère utilisé pour séparer les valeurs entre parenthèses."
L["Number of decimals"] = "Nombre de décimales"
L["Controls the way percentages are displayed."] = "Contrôle la façon dont les pourcentages sont affichés."
L["Data Feed"] = "Flux de données"
L.opt_feed_desc = "Choisissez le flux de données à afficher sur le greffon DataBlocker. Ceci nécessite un addon d'affichage LDB, tel que Titan Panel."
L["Time Measure"] = "Mesure du temps"
L["Activity Time"] = "Temps d'activité"
L["Effective Time"] = "Temps effectif"
L.opt_timemesure_desc = [=[|cFFFFFF00Activité|r : le chronomètre de chaque membre du raid est mis en pause si son activité est interrompue et reprend le compte quand l'activité reprend, méthode commune de mesure des DPS et HPS.

|cFFFFFF00Effective|r : utilisée pour les classements, cette méthode utilise le temps de combat écoulé pour mesurer le DPS et le HPS de tous les membres du raid.]=]
L["Number set duplicates"] = "Nombre de doublons"
L["Append a count to set names with duplicate mob names."] = "Ajoute un compteur aux noms d'ensemble ayant des noms de monstres dupliqués."
L["Set Format"] = "Format des ensembles"
L["Controls the way set names are displayed."] = "Contrôle la façon dont les noms d'ensemble sont affichés."
L["Links in reports"] = "Liens dans les rapports"
L["When possible, use links in the report messages."] = "Utiliser des liens dans les messages de rapport quand cela est possible."
L["Segments to keep"] = "Segments à garder"
L["The number of fight segments to keep. Persistent segments are not included in this."] = "Le nombre de segments de combat à garder. Les segments persistants ne sont pas comptés avec."
L["Persistent segments"] = "Segments persistants"
L["The number of persistent fight segments to keep."] = "Le nombre de segments persistants à conserver."
L["Memory Check"] = "Vérification de la mémoire"
L["Checks memory usage and warns you if it is greater than or equal to %dmb."] = "Vérifie l'utilisation de la mémoire et vous avertit si elle est supérieure ou égale à %d Mo."
L["Minimum segment length"] = "Durée minimale des segments"
L["The minimum length required in seconds for a segment to be saved."] = "La durée minimale requise en secondes pour qu'un segment soit enregistré."
L["Update frequency"] = "Fréquence de mise à jour"
L["How often windows are updated. Shorter for faster updates. Increases CPU usage."] = "La fréquence de rafraichissement des fenêtres. Plus court pour des mises à jour plus fréquentes. Augmente l'utilisation du CPU."
-- columns
L["Columns"] = "Colonnes"
-- tooltips
L["Tooltips"] = "Bulles d'aide"
L["Show Tooltips"] = "Afficher les bulles"
L["Shows tooltips with extra information in some modes."] = "Affiche les bulles d'aide contenant des informations supplémentaires dans certains modes."
L["Informative Tooltips"] = "Bulles d'aide détailées"
L["Shows subview summaries in the tooltips."] = "Affiche le résumé des sous-vues dans les bulles d'aide."
L["Subview Rows"] = "Lignes de sous-vue"
L["The number of rows from each subview to show when using informative tooltips."] = "Le nombre de rangées de chaque sous-vue afficher lors de l'utilisation des bulles d'aide informatives."
L["Tooltip Position"] = "Position de la bulle d'aide"
L["Position of the tooltips."] = "La position des bulles d'aide."
L["Top Right"] = "En haut à droite"
L["Top Left"] = "En haut à gauche"
L["Bottom Right"] = "En bas à droite"
L["Bottom Left"] = "En bas à gauche"
L["Smart"] = "Intelligent"
L["Follow Cursor"] = "Suivre le curseur"
L["Top"] = "Haut"
L["Bottom"] = "Bas"
L["Right"] = "Droite"
L["Left"] = "Gauche"
-- disabled modules
L["Modules"] = "Modules"
L["Disabled Modules"] = "Gestion des modules"
L["Modules Options"] = "Options des modules"
L["Tick the modules you want to disable."] = "Cocher les modules que vous souhaitez désactiver."
L["This change requires a UI reload. Are you sure?"] = "Ce changement nécessite un rechargement de l'IU. Êtes-vous sûr?"
-- themes options
L["Theme"] = "Thème"
L["Themes"] = "Thèmes"
L["Apply Theme"] = "Appliquer le thème"
L["Theme applied!"] = "Thème appliqué!"
L["Name of your new theme."] = "Le nom de votre nouveau thème."
L["Save Theme"] = "Sauvegarde le thème"
L["Delete Theme"] = "Supprimer le thème"
L["Are you sure you want to delete this theme?"] = "Êtes-vous sûr de vouloir supprimer ce thème?"
-- scroll options
L["Scroll"] = "Défilement"
L["Wheel Speed"] = "Vitesse de défilement"
L.opt_wheelspeed_desc = "Change la vitesse de défilement lorsque vous utilisez la roulette de la souris sur la fenêtre."
L["Scroll Icon"] = "Icône de défilement"
L["Scroll mouse button"] = "Bouton de défilement de la souris"
-- minimap button
L["Skada Summary"] = "Résumé Skada"
L["|cffeda55fLeft-Click|r to toggle windows."] = "|cffeda55fClic-gauche|r pour basculer l'affichage des fenêtres."
L["|cffeda55fCtrl+Left-Click|r to show/hide windows."] = "|cffeda55fCtrl+clic-gauche|r pour afficher/cacher les fenêtres."
L["|cffeda55fShift+Left-Click|r to reset."] = "|cffeda55fShift+clic-gauche|r pour réinitialiser."
L["|cffeda55fRight-Click|r to open menu."] = "|cffeda55fClic-droit|r pour ouvrir le menu."
-- skada menu
L["Skada Menu"] = "Menu Skada"
L["Select Segment"] = "Choix de segment"
L["Delete Segment"] = "Suppr. un segment"
L["Keep Segment"] = "Garder un segment"
L["Toggle Windows"] = "Basculer l'affichage"
L["Show/Hide Windows"] = "Afficher/Cacher les fenêtres"
L["Start New Segment"] = "Nouveau segment"
L["Start New Phase"] = "Nouvelle phase"
L["Select All"] = "Tout sélectionner"
L["Deselect All"] = "Tout déselectionner"
-- window buttons
L["Configure"] = "Configurer"
L["Open Config"] = "Ouvrir la configuration"
L.btn_config_desc = "Ouvre la fenêtre de configuration."
L.btn_reset_desc = "Réinitialise les données de tous les combats sauf ceux que vous avez choisi de garder."
L["Segment"] = "Segment"
L["Jump to a specific segment."] = "Vous permet de sélectionner un segment spécific."
L.btn_segment_desc = "Vous permet de sélectionner un segment spécific.\n|cff00ff00Clic du milieu|r pour sélectionner le segment |cffffbb00Actuel|r."
L["Mode"] = "Mode"
L["Jump to a specific mode."] = "Vous permet de sélectionner un mode spécific."
L["Report"] = "Rapport"
L.btn_report_desc = "Ouvre la fenêtre qui vous permet de rapporter les données aux autres de différentes manières.\n|cff00ff00Shift-clic|r pour un Rapport Rapide."
L["Stop"] = "Arrêter/Reprendre"
L.btn_stop_desc = "Arrête ou reprend le segment en cours. Utile pour arrêter la collecte des données après un wipe. L'arrêt automatique peut être activé dans les paramètres."
L["Segment Stopped."] = "Segment arrêté."
L["Segment Paused."] = "Segment suspendu."
L["Segment Resumed."] = "Segment repris."
L["Quick Access"] = "Accès rapide"
-- default segments
L["Total"] = "Total"
L["Current"] = "Actuel"
-- report module and window
L["Skada: %s for %s:"] = "Skada: %s pour %s:"
L["Self"] = "Soi-même"
L["Whisper Target"] = "Chuchoter à la cible"
L["Line"] = "Ligne"
L["Lines"] = "Lignes"
L["There is nothing to report."] = "Il n'y a rien à rapporter."
L["No mode or segment selected for report."] = "Aucun mode ou segment n'a été sélectionné pour le rapport."
-- Bar Display Module --
L["Bar display"] = "Affichage à barres"
L.mod_bar_desc = "L'affichage à barres est la fenêtre de barres habituellement utilisée par la plupart des damage meters. Il peut être personnalisé de façon approfondie."
-- Threat Module --
L["Threat"] = "Menace"
L["Threat Warning"] = "Avertissement de la menace"
L["Flash Screen"] = "Flasher l'écran"
L["This will cause the screen to flash as a threat warning."] = "Ceci fera clignoter l'écran pour en faire un avertissement sur la menace."
L["Shake Screen"] = "Secouer l'écran"
L["This will cause the screen to shake as a threat warning."] = "Ceci fera secouer l'écran pour en faire un avertissement sur la menace."
L["Warning Message"] = "Message d'avertissement"
L["Print a message to screen when you accumulate too much threat."] = "Affiche un message à l'écran quand vous avez accumulé trop de menace."
L["Play sound"] = "Jouer un son"
L["This will play a sound as a threat warning."] = "Ceci jouera un son pour en faire un avertissement sur la menace"
L["Message Output"] = "Sortie des avertissements"
L["Choose where warning messages should be displayed."] = "Choisissez où les messages d'avertissement doivent être affichés."
L["Chat Frame"] = "Fenêtre de discussion"
L["Blizzard Error Frame"] = "Cadre des erreurs"
L["Threat sound"] = "Son de menace"
L.opt_threat_soundfile_desc = "Le son qui sera joué chaque fois que votre pourcentage de menace atteint un certain point."
L["Warning Frequency"] = "Fréquence des alertes"
L["Threat Threshold"] = "Seuil de menace"
L.opt_threat_threshold_desc = "Quand votre menace atteint ce niveau par rapport au tank, les avertissements sont affichés."
L["Show raw threat"] = "Menace brute"
L.opt_threat_rawvalue_desc = "Affiche le pourcentage brut de menace par rapport au tank au lieu de celui modifié selon la portée."
L["Use focus target"] = "Cible de la focalisation"
L.opt_threat_focustarget_desc = "Demande à Skada de vérifier également votre focalisation et la cible de votre focalisation (dans cet ordre) pour l'affichage de la menace."
L["Disable while tanking"] = "Désactiver en tankant"
L.opt_threat_notankwarnings_desc = "Ne donne aucun avertissement si vous êtes en posture défensive, en forme d'ours, sous fureur vertueuse ou en présence de givre."
L["Ignore Pets"] = "Ignorer les familiers"
L.opt_threat_ignorepets_desc = "Désactive la menace des familiers et ne surveille que celle des joueurs."
L["> Pull Aggro <"] = "> Reprise d'aggro <"
L["Show Pull Aggro Bar"] = "Barre reprise d'aggro"
L.opt_threat_showaggrobar_desc = "Affiche une barre indiquant la quantité de menace que vous avez besoin de faire pour reprendre l'aggro."
L["Hide empty window"] = "Cacher la fenêtre vide"
L.opt_threat_hideempty_desc = "Cacher complètement la fenêtre lorsqu'il n'y a rien à afficher."
L["Test Warnings"] = "Test d'avertissement"
L["TPS"] = "MPS"
L["Threat: Personal Threat"] = "Menace : Menace perso."
-- Absorbs & Healing Module --
L["Healing"] = "Soins prodigués"
L["Healing Done"] = "Soins prodigués"
L["Healing Taken"] = "Soins reçus"
L["Healed target list"] = "Cibles soignées"
L["Healing spell list"] = "Sorts de soin"
L["%s's healing"] = "Soins prodigués par %s"
L["%s's healed targets"] = "Cibles soignées par %s"
L["actor heal spells"] = function(n1, n2) return ((not n2 or n1 == n2) and "Sorts de soin \1242 %s" or "Soins \1242 %s sur %s"):format(n1, n2) end
L["HPS"] = "SPS"
L["sHPS"] = "SPS (sous-vues)"
L["Healing: Personal HPS"] = "Soins prodigués: SPS perso."
L["RHPS"] = "RSPS"
L["Healing: Raid HPS"] = "Soins prodigués: SPS du raid"
L["Total Healing"] = "Total des soins"
L["Overheal"] = "Soins excessifs"
L["Overhealing"] = "Soins excessifs"
L["Overhealed target list"] = "Cibles soignées en excès"
L["Overheal spell list"] = "Sorts d'excès de soin"
L["%s's overheal targets"] = "Cibles soignées en excès par %s"
L["actor overheal spells"] = function(n1, n2) return ((not n2 or n1 == n2) and "Sorts d'excès de soin \1242 %s" or "Soins excessifs \1242 %s sur %s"):format(n1, n2) end
L["Absorbs"] = "Absorptions"
L["Absorbed target list"] = "Cibles absorbées"
L["Absorb spell list"] = "Sorts d'absorption"
L["%s's absorbed targets"] = "Cibles absorbées par %s"
L["actor absorb spells"] = function(n1, n2) return ((not n2 or n1 == n2) and "Sorts d'absorption \1242 %s" or "Absorptions \1242 %s sur %s"):format(n1, n2) end
L["APS"] = "APS"
L["sAPS"] = "APS (sous-vues)"
L["Absorbs and Healing"] = "Absorptions et soins"
L["Absorbs and healing spells"] = "Sorts d'absorption et de soin"
L["Absorbed and healed targets"] = "Cibles absorbées et soignéees"
L["%s's absorbed and healed targets"] = "Cibles absorbées et soignées par %s"
L["actor absorb and heal spells"] = function(n1, n2) return ((not n2 or n1 == n2) and "Sorts d'absorption et de soin \1242 %s" or "Absorptions et soins \1242 %s sur %s"):format(n1, n2) end
L["Healing source list"] = "Sources de soin"
L["%s's received healing"] = "Soins reçus par %s"
L["Healing Done By Spell"] = "Soins prodigués par sort"
L["Healing spell sources"] = "Sources du sort de soin"
-- Auras Module --
L["Uptime"] = "Disponibilité"
L["Buffs and Debuffs"] = "Buffs et Debuffs"
L["Buffs"] = "Buffs"
L["Buff spell list"] = "Liste des buffs"
L["%s's buffs"] = "Buffs \1242 %s"
L["Debuffs"] = "Debuffs"
L["Debuff spell list"] = "Liste des debuffs"
L["Debuff target list"] = "Cibles des debuffs"
L["actor debuffs"] = function(n1, n2) return ((not n2 or n1 == n2) and "Debuffs \1242 %s" or "Debuffs \1242 %s sur %s"):format(n1, n2) end
L["%s's <%s> targets"] = "Cibles \1242 <%2$s> \1242 %1$s"
L["Sunder Counter"] = "Fracasser armure"
L["Sunder target list"] = "Cibles de Fracasser armure"
-- CC Tracker Module --
L["Crowd Control"] = "Contrôle de foule"
L["CC Done"] = "Contrôles effectués"
L["CC Taken"] = "Contrôles subis"
L["CC Breaks"] = "Contrôle brisés"
L["Crowd Control Spells"] = "Sorts de contrôle"
L["Crowd Control Targets"] = "Cibles de contrôle"
L["Crowd Control Sources"] = "Sources de contrôle"
L["%s's control spells"] = "Sorts de contrôle \1242 %s"
L["%s's control targets"] = "Cibles de contrôle \1242 %s"
L["%s's control sources"] = "Sources de contrôle \1242 %s"
L["Ignore Main Tanks"] = "Ignorer tanks principaux"
L["%s on %s removed by %s"] = "%s sur %s enlevé(e) par %s"
L["%s on %s removed by %s's %s"] = "%s sur %s enlevé(e) par %s avec %s"
-- Damage Module --
-- damage done module
L["Damage"] = "Dégâts infligés"
L["Damage target list"] = "Cibles des dégâts"
L["Damage spell list"] = "Sorts des dégâts"
L["Damage spell details"] = "Détails du sort de dégât"
L["Damage spell targets"] = "Cibles du sort de dégât"
L["Damage Done"] = "Dégâts infligés"
L["actor damage"] = function(n1, n2) return ((not n2 or n1 == n2) and "Dégâts \1242 %s" or "Dégâts \1242 %s sur %s"):format(n1, n2) end
L["%s's <%s> damage"] = "Dégâts \1242 %s par <%s>"
L["Useful Damage"] = "Dégâts utiles"
L["Useful damage on %s"] = "Dégâts utiles sur %s"
L["Damage Done By Spell"] = "Dégâts infligés par sort"
L["%s's sources"] = "Source \1242 %s"
L["DPS"] = "DPS"
L["sDPS"] = "DPS (sous-vues)"
L["Damage: Personal DPS"] = "Dégâts : DPS personnel"
L["RDPS"] = "RDPS"
L["Damage: Raid DPS"] = "Dégâts : DPS du raid"
L["Absorbed Damage"] = "Dégâts absorbés"
L["Enable this if you want the damage absorbed to be included in the damage done."] = "Si activé, les dégâts absorbés seront inclus dans les dégâts infligés."
-- damage taken module
L["Damage Taken"] = "Dégâts subis"
L["Damage taken by %s"] = "Dégâts subis par %s"
L["Damage source list"] = "Sources des dégâts"
L["Damage spell sources"] = "Sources du sort de dégât"
L["Damage Taken By Spell"] = "Dégâts subis par sort"
L["%s's targets"] = "Cibles \1242 %s"
L["DTPS"] = "DSPS"
L["sDTPS"] = "DSPS (sous-vues)"
-- enemy damage done module
L["Enemies"] = "Ennemis"
L["Enemy Damage Done"] = "Dégâts infligés (Ennemi)"
L["Damage done per player"] = "Dégâts infligés par joueur"
L["Damage from %s"] = "Dégâts subis par %s"
-- enemy damage taken module
L["Enemy Damage Taken"] = "Dégâts subis (Ennemi)"
L["%s's damage sources"] = "Sources des dégâts subis par %s"
L["%s below %s%%"] = "%s à partir \1242 %s%%"
L["%s - %s%% to %s%%"] = "%s - %s%% à %s%%"
L["Phase %s"] = "Phase %s"
L["%s - Phase %s"] = "%s - Phase %s"
L["%s - Phase 1"] = "%s - Phase 1"
L["%s - Phase 2"] = "%s - Phase 2"
L["%s - Phase 3"] = "%s - Phase 3"
-- enemy healing done module
L["Enemy Healing Done"] = "Soins prodigués (Ennemi)"
-- avoidance and mitigation module
L["Avoidance & Mitigation"] = "Évitement et mitigation"
L["Damage Breakdown"] = "Détails des dégâts"
L["%s's damage breakdown"] = "Détails des dégâts \1242 %s"
-- friendly fire module
L["Friendly Fire"] = "Dégâts aux alliés"
-- useful damage targets
L["Important targets"] = "Cibles importantes"
L["Oozes"] = "Limons"
L["Princes overkilling"] = "Dégât exagéré sur les Princes"
-- L["Adds"] = ""
L["Halion and Inferno"] = "Halion et Inferno"
L["Valkyrs overkilling"] = "Dégât exagéré sur les Valkyrs"
-- Deaths Module --
L["%s's death"] = "Mort \1242 %s"
L["%s's deaths"] = "Morts \1242 %s"
L["Death log"] = "Journal de mort"
L["%s's death log"] = "Journal de mort \1242 %s"
L["Player's deaths"] = "Morts du joueur"
L["%s dies"] = "%s meurt"
L["Spell details"] = "Détails du sort"
L["Spell"] = "Sort"
L["Amount"] = "Montant"
L["Source"] = "Source"
L["Change"] = "Changement"
L["Survivability"] = "Durée de vie"
L["Events Amount"] = "Nombre d'événments"
L["Set the amount of events the death log should record."] = "Choisissez le nombre d'événments que le journal de mort doit enregistrer."
L["Minimum Healing"] = "Minimum des soins"
L["Ignore heal events that are below this threshold."] = "Ignorer tous les soins prodigués inférieurs à ce seuil."
L["Announce Deaths"] = "Annoncer les morts"
L["Announces information about the last hit the player took before they died."] = "Annonce le dernier coup que la personne a reçu avant de mourir."
-- activity module
L["Activity"] = "Activité"
-- dispels module lines --
L["Dispel spell list"] = "Sorts de dissipation"
L["Dispelled spell list"] = "Sorts dissipés"
L["Dispelled target list"] = "Cibles dissipées"
L["%s's dispel spells"] = "Sorts de dissipation \1242 %s"
L["%s's dispelled spells"] = "Sorts dissipés par %s"
L["%s's dispelled targets"] = "Cibles dissipées par %s"
-- failbot module lines --
L["Fails"] = "Échecs"
L["%s's fails"] = "Échecs \1242 %s"
L["Player's failed events"] = "Échecs du joueur"
L["Event's failed players"] = "Joueurs échoués"
L["Report Fails"] = "Rapporter les échecs"
L["Reports the group fails at the end of combat if there are any."] = "Rapporte les échecs du groupe à la fin du combat."
L["Ignored Events"] = "Événements ignorés"
-- interrupts module lines --
L["Interrupt spells"] = "Sorts d'interruption"
L["Interrupted spells"] = "Sorts interrompus"
L["Interrupted targets"] = "Cibles interrompues"
L["%s's interrupt spells"] = "Sorts d'interruption \1242 %s"
L["%s's interrupted spells"] = "Sorts interrompus par %s"
L["%s's interrupted targets"] = "Cibles interrompues par %s"
L["%s interrupted!"] = "%s interrompu!"
-- Power gained module --
L["Resources"] = "Ressources"
L["Power gained: %s"] = "Ressources: %s"
L["%s gained spells"] = "Sorts de gain \1242 %s"
L["%s's gained %s"] = "Gains \1242 %2$s \1242 %1$s"
-- Parry module lines --
L["Parry-Haste"] = "Parade-hâte"
L["Parry target list"] = "Cibles de parade"
L["%s's parry targets"] = "Cibles parées \1242 %s"
L["%s parried %s (%s)"] = "%s a paré %s (%s)"
-- Potions module lines --
L["Potions"] = "Potions"
L["Potions list"] = "Liste des potions"
L["Players list"] = "Liste des joueurs"
L["%s's used potions"] = "Potions utilisées par %s"
L["Pre-potion"] = "Pre-potion"
L["pre-potion: %s"] = "pré-potion: %s"
L["Prints pre-potion after the end of the combat."] = "Affiche, en fin du combat, la liste des joueurs ayant utilisé des potions avant le début du combat."
-- healthstone --
L["Healthstones"] = "Pierres de soins"
-- resurrect module lines --
L["Resurrects"] = "Résurrections"
L["Resurrect spell list"] = "Sorts de résurrection"
L["Resurrect target list"] = "Cibles ressuscitées"
L["%s's resurrect spells"] = "Sorts de résurrection \1242 %s"
L["%s's resurrect targets"] = "Cibles ressuscitées par %s"
-- nickname module lines --
L["Nickname"] = "Surnom"
L["Nicknames are sent to group members and Skada can use them instead of your character name."] = "Les surnoms sont envoyés aux membres de votre groupe et Skada pourra l'utilise à la place du nom de votre personnage."
L["Set a nickname for you."] = "Définissez un surnom pour vous."
L["Nickname isn't a valid string."] = "Le surnom contient des caractères invalides."
L["Your nickname is too long, max of 12 characters is allowed."] = "Votre surnom est trop long, un maximum de 12 caractères est autorisé."
L["Only letters and two spaces are allowed."] = "Seules les lettres et deux espaces sont autorisés."
L["Your nickname contains a forbidden word."] = "Votre surnom contient un mot interdit."
L["You can't use the same letter three times consecutively, two spaces consecutively or more then two spaces."] = "Vous ne pouvez pas utiliser la même lettre trois fois de suite, deux espaces consécutifs ou plus de deux espaces."
L["Ignore Nicknames"] = "Ignorer les surnoms"
L["When enabled, nicknames set by Skada users are ignored."] = "Si activé, les surnoms définis par les autres utilisateur Skada sont ignorés."
L["Name display"] = "Format des noms"
L["Choose how names are shown on your bars."] = "Choisissez comment les noms seront affichés sur les barres."
L["Clear Cache"] = "Vider le cache"
L["Are you sure you want clear cached nicknames?"] = "Êtes-vous sûr vouloir vider le cache des surnoms?"
-- overkill module lines --
L["Overkill"] = "Dégâts excessifs"
L["Overkill spell list"] = "Sorts des dégâts excessifs"
L["Overkill target list"] = "Cibles des dégâts excessifs"
L["%s's overkill spells"] = "Sorts des dégâts excessifs \1242 %s"
L["%s's overkill targets"] = "Cibles des dégâts excessifs \1242 %s"
-- tweaks module lines --
L["Improvement"] = "Amélioration"
L["Tweaks"] = "Ajustements"
L["First hit"] = "Première attaque"
L["|cffffff00First Hit|r: %s from %s"] = "|cffffff00Première attaque|r: %s \1242 %s"
L["|cffffbb00First Hit|r: *?*"] = "|cffffbb00Première attaque|r: *?*"
L["|cffffbb00Boss First Target|r: %s"] = "|cffffbb00Première cible de boss|r: %s"
L.opt_tweaks_firsthit_desc = "Affiche la première attaque causant le début de combat.\nFonctionne uniquement contre les boss."
L["Filter DPS meters Spam"] = "Filtrer les messages DPS"
L.opt_tweaks_spamage_desc = "Supprime les messages provenant des damage meters et les remplace par un lien unique affichant les statistics dans une popup."
L["Reported by: %s"] = "Rapporté par: %s"
L["Smart Stop"] = "Arrêt intelligent"
L.opt_tweaks_smarthalt_desc = "Arrête automatiquement le segment en cours après la mort du boss.\nUtile pour éviter de collecter des données en cas de bug de combat."
L["Duration"] = "Durée"
L.opt_tweaks_smartwait_desc = "Combien de temps Skada doit attendre avant d'arrêter le segment?"
L["Modes Icons"] = "Icônes des modes"
L["Show modes icons on bars and menus."] = "Afficher les icônes des modes sur les barres et les menus."
L.opt_tweaks_combatlogfix_desc = "Empêche le journal de combat de bugger sans le ronger complètement."
L["Conservative Mode"] = "Mode conservateur"
L["Aggressive Mode"] = "Mode agressif"
L.opt_tweaks_combatlogfixalt_desc = "Effacer constamment le journal de combat au lieu de seulement quand il bugge."
L["%d filtered / %d events found. Cleared combat log, as it broke."] = "%d filtrés / %d événements trouvés. Journal de combat effacé car il a buggé."
L["Enable this if you want to ignore |cffffbb00%s|r."] = "Si activé, |cffffbb00%s|r sera complètement ignoré(e)."
L["Announce %s"] = "Annoncer %s"
L["Announces how long it took to apply %d stacks of %s and announces when it drops."] = "Annonce combien de temps il a fallu pour appliquer %d piles \1242 %s et annonce quand il expire."
L["%s dropped from %s!"] = "%s sur %s a expiré!"
L["%s stacks of %s applied on %s in %s sec!"] = "%s piles \1242 %s appliquées sur %s en %s sec!"
L["My Spells"] = "Mes sorts"
-- arena
L["Custom Arena Colors"] = "Couleurs d'arène personnalisées"
L["Enable this if you want to use custom arena teams colors."] = "Activez cette option si vous souhaitez utiliser des couleurs d'équipes d'arène personnalisées."
L["Gold Team"] = "Équipe dorée"
L["Green Team"] = "Équipe verte"
L["Color for %s."] = "Couleur \1242 %s."
-- notifications
L["Notifications"] = "Notifications"
L.opt_toast_desc = "Utiliser des notifications visuelles au lieu des messages de chat quand cela est possible."
L["Test Notifications"] = "Tester les notifications"
-- comparison module
L["Comparison"] = "Comparaison"
L["Damage Comparison"] = "Comparaison des dégâts"
L["%s vs %s: %s"] = "%s vs %s: %s"
L["%s vs %s: Spells"] = "%s vs %s: Sorts"
L["%s vs %s: Targets"] = "%s vs %s: Cibles"
L["%s vs %s: Damage on %s"] = "%s vs %s: Dégâts sur %s"
-- about
L["About"] = "À Propos"
L["Author"] = "Auteur"
L["Credits"] = "Crédits"
L["Date"] = "Date"
L["Discord"] = "Discord"
L["Donate"] = "Faire un don"
L["License"] = "Licence"
L["Version"] = "Version"
L["Website"] = "Site web"
-- some bosses entries
L["Acidmaw"] = "Gueule-d'acide"
L["Auriaya"] = "Auriaya"
L["Blood Prince Council"] = "Conseil des princes de sang"
L["Champions of the Alliance"] = "Champions de l'Alliance"
L["Champions of the Horde"] = "Champions de la Horde"
L["Cult Adherent"] = "Adhérent du culte"
L["Cult Fanatic"] = "Fanatique du culte"
L["Darnavan"] = "Darnavan"
L["Deformed Fanatic"] = "Fanatique déformé"
L["Dreadscale"] = "Écaille-d'effroi"
L["Empowered Adherent"] = "Adhérent investi"
L["Faction Champions"] = "Champions de faction"
L["Gas Cloud"] = "Nuage de gaz"
L["General Vezax"] = "Général Vezax"
L["Gluth"] = "Gluth"
L["Halion"] = "Halion"
L["Hogger"] = "Lardeur"
L["Ice Sphere"] = "Sphère de glace"
L["Icecrown Gunship Battle"] = "Bataille des canonnières"
L["Icehowl"] = "Glace-hurlante"
L["Kel'Thuzad"] = "Kel'Thuzad"
L["Kologarn"] = "Kologarn"
L["Lady Deathwhisper"] = "Dame Murmemort"
L["Living Inferno"] = "Inferno vivant"
L["Mimiron"] = "Mimiron"
L["Onyxia"] = "Onyxia"
L["Prince Keleseth"] = "Prince Keleseth"
L["Prince Taldaram"] = "Prince Taldaram"
L["Prince Valanar"] = "Prince Valanar"
L["Raging Spirit"] = "Esprit déchaîné"
L["Reanimated Adherent"] = "Adhérent réanimé"
L["Reanimated Fanatic"] = "Fanatique réanimé"
L["Sapphiron"] = "Saphiron"
L["Shambling Horror"] = "Horreur titubante"
L["Sindragosa"] = "Sindragosa"
L["Thaddius"] = "Thaddius"
L["The Four Horsemen"] = "Les quatre cavaliers"
L["The Iron Council"] = "Assemblée du fer"
L["The Lich King"] = "Le roi-liche"
L["The Northrend Beasts"] = "Les bêtes du Norfendre"
L["Thorim"] = "Thorim"
L["Twin Val'kyr"] = "Jumelles val'kyrs"
L["Val'kyr Shadowguard"] = "Garde de l'ombre val'kyr"
L["Valithria Dreamwalker"] = "Valithria Marcherêve"
L["Volatile Ooze"] = "Limon volatil"
L["Wicked Spirit"] = "Esprit malfaisant"
L["Yogg-Saron"] = "Yogg-Saron"