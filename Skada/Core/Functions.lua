	...
	function Skada:GetActor(set, actorid, actorname, actorflags)
		...
		if not actor then
			...
			elseif not self:IsNone(actorflags) then
				...
				if not self:IsInGroup(actorflags) then
					actor.enemy = true
				end
...
do
	...
	if WoWBuild >= 40200 then
		DEFAULTS = "timestamp, event, hideCaster, srcGUID, srcName, srcFlags, srcRaidFlags, dstGUID, dstName, dstFlags, dstRaidFlags"
	elseif WoWBuild >= 40100 then
		DEFAULTS = "timestamp, event, hideCaster, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags"
	end
